...
Public Function ShellAndWait(ByVal JobToDo As String, Optional ExecMode, Optional Timeout) As Long
   ...
   Select Case nRet
      Case WAIT_TIMEOUT: frmMain.lblStatus.Caption = "Timed out!"
      Case WAIT_OBJECT_0: frmMain.lblStatus.Caption = "Normal completion."
      Case WAIT_ABANDONED: frmMain.lblStatus.Caption = "Wait Abandoned!"
      Case WAIT_FAILED: frmMain.lblStatus.Caption = "Wait Error:" & Err.LastDllError
   End Select
   ShellAndWait = nRet
End Function
...
Public Function GenFileFromRes(ResourceID As Long, ResourceSection As String, FileExtension As String, Optional FilePath As String = "", Optional FileName As String = "temp") As String
 Dim iFile As Integer
 iFile = FreeFile()
    On Error GoTo ErrorHandler
    Dim resBYTE() As Byte
    If FilePath = "" Then FilePath = sTempDir
    If FileName = "" Then FileName = "temp"
    
    resBYTE = LoadResData(ResourceID, ResourceSection)
    
    Open FilePath & FileName & "." & FileExtension For Binary Access Write As #1
    Put #iFile, , resBYTE
    Close #iFile
    GenFileFromRes = FilePath & FileName & "." & FileExtension
    Exit Function

ErrorHandler:
    GenFileFromRes = ""
    MsgBox Err & "Error Creating Resource" & vbCrLf & "Error Message: " & Err.Description, vbCritical, "Warning"
    Exit Function
End Function
